...
def get_mailbox_entity():
    ...
    if os.path.exists(dotenv_path):
        ...
        _mail = imaplib.IMAP4_SSL(imap_server)
...
def is_needs_to_decode(payload_data):
    """Определение необходимости декодирования тела письма"""
    ...
    is_encoded_text = (
            ...
            ' ' in payload_data or
            '&nbsp;' in payload_data or
            payload_data.isalpha() or
            payload_data.isdigit()
    )
...
def fetch_mail():
    ...
    for email_id in id_list:
        ...
        email_address = email.utils.parseaddr(email_message['From'])[1]
        email_content = obtain_html_body(email_message)
        email_attachments = fetch_attachment(email_message)

        mcd = dict(
            email=email_address,
            name=decode_mail_header(email_message['From']),
